...
라벨만 고칠 거면 `useHydrated` 로 충분하지만, 그러면 FOUC 이 남는다. 두 증상의 뿌리가
하나(서버가 알 수 없는 저장소를 첫 렌더의 입력으로 삼은 것)이므로 저장소를 옮겼다.

- `src/app/layout.tsx` 가 `await cookies()` 로 읽어 `<html style>` 에 CSS 변수를 실어
  보낸다. **첫 페인트부터 올바른 색이다**
- `ThemeProvider` 는 그 값을 `initialThemeId` 로 받는다. 클라이언트는 저장소를 직접
  읽지 않으므로 첫 렌더의 입력이 서버와 같다
- 전환 시 `useEffect` 가 CSS 변수를 다시 주입하고 쿠키를 갱신한다

**대가 1 — `/_not-found` 가 동적이 됐다.** 루트 레이아웃이 요청 시점 API(`cookies()`)를
쓰면 전 라우트가 동적 렌더링으로 내려간다. 이 앱은 이미 실제 페이지가 전부 동적이라
실질 변화는 `/_not-found`·`/_global-error` 뿐이고, 렌더 비용은 무시할 만하다.

**대가 2 — 쿠키가 요청마다 실린다.** 값은 테마 id 하나(`base`|`emerald`|`indigo`|`red`)라
수십 바이트다. `httpOnly` 는 쓰지 않는다 — 클라이언트가 전환 시 직접 써야 한다.

**하지 않은 것 — localStorage 값 이관.** 쿠키 도입 이전 사용자는 테마를 한 번 다시
골라야 한다. 마운트 후 이관하려면 이펙트 안에서 `setState` 를 해야 하는데
`react-hooks/set-state-in-effect` 가 이를 에러로 막는다(연쇄 렌더). 규칙을 우회하거나
인라인 스크립트를 새로 들이는 것보다, 한 번 다시 고르는 편이 싸다고 판단했다.
...
헤더의 `오늘` 라벨은 원래 `format(new Date(), ...)` 를 서버·클라이언트 양쪽에서 그대로
렌더했다. 일간 플래너가 쓰는 "유효 오늘"(`src/lib/task-dates.ts`, 하루 시작 시각 이전이면
전날)을 헤더만 몰라서 자정~하루 시작 시각 사이에는 같은 화면에 두 날짜가 떴다.

`useEffectiveToday()` 로 기준을 맞추면 입력이 둘 다 서버가 알 수 없는 값이 된다.

- 현재 시각 — 서버 TZ 가 클라이언트와 다르면 날짜 자체가 갈린다
- 프로필 — 사례 1 과 같은 스트리밍 쿼리라 도착 타이밍이 갈린다

그래서 라벨을 이미 클라이언트 전용인 `now` 에 얹었다. `now` 가 `null` 인 첫 렌더는
서버·클라이언트가 같고, 마운트 후에야 날짜가 채워진다.
...
- **`useHydrated` 를 덧대지 않았다.** `now === null` 게이트가 첫 렌더를 이미 고정한다.
  `ready` 는 마운트 이후에만 라벨에 닿는다
- **프로필 도착 전에는 달력 오늘로 폴백한다.** 헤더는 셸이라 로딩 문구를 넣을 자리가
  없고, `!ready` 일 때 기본값 `06:00` 으로 계산하면 틀린 날짜를 자신 있게 보여주게 된다.
  로드 실패(`failed`)는 `useEffectiveToday` 가 이미 `'00:00'` 으로 달력 오늘을 돌려주므로
  헤더에 분기가 없다 — 실패의 표면화는 `/daily` 의 배너가 맡는다.
  **한계: `/daily` 를 열지 않으면 실패가 어디에도 드러나지 않는다.** 헤더는 셸이라
  에러 UI 를 놓을 자리가 없고 실패해도 유효한 값(달력 오늘)을 보여주므로 감수한
  트레이드오프다 — 헤더에 재시도 버튼이 없는 이유가 이것이다
- **대가 — 첫 페인트에 날짜가 없다.** 라벨 `<p>` 에 `min-h-5` 로 한 줄 높이를 예약해
  채워질 때 헤더 행이 밀리지 않게 했다
...
두 번째 명령이 어느 컴포넌트인지 알려준다. **2026-08-18 기준 0건이다.**

로그 세기는 여전히 수동 확인 항목이다 — 경고가 stderr 로만 나오고 스펙은 통과한다.
사례 2 는 자동 가드가 있다: `e2e/theme.spec.ts` 가 비기본 테마 쿠키로 진입해
`pageerror` 를 수집하고, 서버가 보낸 HTML 에 그 테마 라벨이 들어 있는지 단언한다.
새 사례를 고칠 때도 로그 grep 이 아니라 이런 스펙으로 고정하는 편이 낫다.
