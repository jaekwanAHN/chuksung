...
`PERF_TEST_USER_*` 가 없으면 perf는 인증 전에 실패한다. E2E 폴백은 볼륨 고정 계약과
양립할 수 없어 #148에서 제거했다. 작업 워크트리에는 두 값을 복사하지 않으며 perf
스크립트만 기본 체크아웃에서 런타임에 읽는다 (`measurement-contract.md`).
...
`volume.mjs`의 볼륨 경고는 데이터 시딩이 코드 회귀로 오해된 사건
(`incidents/data-volume-contamination.md`) 때문에 넣은 장치인데, 계정을 공유하는 한
**그 경고가 뜰 조건이 상시로 만들어진다.**
...
스냅샷에 `account` 필드를 남긴다. 새 측정은 `perf`만 만들 수 있지만 과거 `e2e`·미기록
스냅샷은 원장에 남아 있다. 직전 회차의 양쪽 모두 `perf`가 아니면 색상 델타를 만들지
않고 새 회차를 **새 기준선**으로 삼는다.

## 범위 밖

- **원장의 브랜치 간 공유** — 스냅샷은 현재 체크아웃에 속하고 git SHA·브랜치를
  기록하지만, 동일 브랜치 안에서도 전후 코드가 달라지는 것이 정상이라 SHA 자체를
  비교 차단 조건으로 쓰지는 않는다 (`measurement-contract.md`)
- **perf 병렬 실행** — 계정을 나눠도 머신과 Supabase 인스턴스가 하나라 성립하지 않는다.
  `.perf.lock`은 병렬화가 아니라 동시 실행 거부 장치다
