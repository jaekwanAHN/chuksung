...
test.describe('템플릿 관리 · 하루 시작 시각', () => {
  ...
  test('템플릿을 추가하면 오늘 일간 목록에 시딩되고, 삭제까지 정리된다', async ({
    ...
    // 일간 목록에 시딩된 태스크 확인 (템플릿 추가 POST 가 그 자리에서 시딩하고
    // 결과를 응답에 실어 온다 — 재조회 없음, #75)
  ...
  /**
   * `useTaskTemplates` 의 `applySeeding` 이 진행 중인 일간 조회를 먼저 끊는 것을
   * 고정한다. 그 취소를 지우면 이 테스트만 실패한다 — 위의 "템플릿을 추가하면…" 은
   * 통과하므로 이 스펙 없이는 무방비다.
   *
   * POST 응답이 시딩 결과를 실어 오지만(#75) 그것만으로는 부족하다. **뮤테이션
   * 이전에 이미 날아간 조회**는 템플릿이 존재하기 전에 계산된 목록을 들고 늦게
   * 도착해, 캐시에 쓴 값을 덮어쓴다. 여기서는 첫 로딩이 아직 끝나지 않은 상태라
   * 병합할 캐시가 없어 취소 후 재조회 경로를 탄다 (docs/task-race-guards.md).
   *
