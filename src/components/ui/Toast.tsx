 ...
 * - bottom(기본)은 모바일 하단 탭바 위에 위치하도록 `bottom-20 md:bottom-6` 사용
 * - animate-slide-up/down 은 globals.css 에 정의된 @utility
 */
export function Toast({
  ...
  return (
    ...
    <div className={cn('fixed left-1/2 z-50 -translate-x-1/2', positionStyles[position])}>
      <div
        role="status"
        aria-live="polite"
        className={cn(
          'flex items-center gap-3 whitespace-nowrap rounded-2xl px-5 py-3 shadow-xl',
          position === 'top' ? 'animate-slide-down' : 'animate-slide-up',
