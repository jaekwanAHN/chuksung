-- 베이스 스키마 백필 (평가리포트 §5-2)
--
-- 마이그레이션 히스토리가 0001_quiz부터 시작해 신규 환경을 마이그레이션만으로
-- 재구축할 수 없던 문제를 해결한다. 이 파일은 0001 이전에 Supabase SQL Editor로
-- 수동 적용됐던 상태(profiles/tasks/뷰/ddays/job_postings)를 그대로 담는다.
--
-- ⚠️ 기존(운영) DB에는 이미 적용된 상태이므로 실행하면 안 된다.
--    `supabase migration repair --status applied 0000` 으로 히스토리에만 기록할 것.
--    (goals·task_templates·quiz는 0001~0005, security_invoker는 0006 참조.
--     profiles.day_start_time은 0004에서 추가.)
...
-- 3. 기록 뷰 (완료된 태스크만) — security_invoker는 0006에서 적용
