...
- 새 이슈는 **문제·영향·관측 증거·기대 결과**를 중심으로 작성한다. 원인 규명은 등록 조건이
  아니다. 확인한 원인은 증거와 함께 참고에 남기고, 가설은 제목·현상에서 사실로 단정하지 않는다.
- 완료 조건은 해결 후 확인할 결과로 적는다. 제약·범위 제외는 승인된 요구사항이나 회귀 방지
  근거가 있을 때만 출처와 함께 적는다. 현재 구현이나 실측값만으로 제품 요구·해결 기술을
  확정하지 않고, 공통 하드 룰은 복사하지 않는다.
- 착수 질문·해결 제안은 기본 항목으로 채우지 않는다. 이미 확보한 중요한 조사 기록만 참고에
  남기며 사실·가설·미승인 후보·승인된 결정을 구분한다. 구현 방법과 구체 검증 절차는
  `/work`의 계획 단계에서 정한다. 작성 배경은 `docs/work-evidence-routing.md`를 참조한다.
- 이슈의 **완료 조건과 명시된 제약**을 기준으로 계획한다. 해결 방향과 구현 체크리스트는
  승인 여부를 확인하고, 미승인 제안은 현재 증거에 따라 재검토한다. 기존 `할 일`도 결과·제약·
  구현 제안으로 구분해 읽는다. 상세 구현 체크리스트는 계획 승인 뒤 기록하고 승인 근거를 남긴다.
