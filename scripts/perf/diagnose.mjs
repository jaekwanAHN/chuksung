...
import { assertLighthouseResult } from './lighthouse-result.mjs'
...
async function diagnose() {
  ...
  const profileDir = fs.mkdtempSync(
    path.join(os.tmpdir(), 'chuksung-perf-diagnose-')
  )
  ...
  await context.addCookies(
    toBrowserCookies(cookies, `http://localhost:${PORT}`)
  )

  try {
    for (const page of PAGES) {
      console.log(`\n${'='.repeat(70)}\n▸ ${page}\n${'='.repeat(70)}`)
      const requestedUrl = `http://localhost:${PORT}${page}`
      ...
      assertLighthouseResult(lhr, requestedUrl)
      const a = lhr.audits
      fs.writeFileSync(
      ...
      console.log(
        `Perf ${Math.round(lhr.categories.performance.score * 100)} · ` +
          ...
          `LCP ${(a['largest-contentful-paint'].numericValue / 1000).toFixed(
            2
          )}s`
      )
      ...
      for (const it of a['mainthread-work-breakdown']?.details?.items ?? []) {
        if (it.duration >= 20)
          console.log(`  ${ms(it.duration).padStart(8)}  ${it.group}`)
      }
      ...
      for (const it of boot.slice(0, 12)) {
        ...
        console.log(
          `  total ${ms(it.total).padStart(8)} | eval ${ms(
            it.scripting
          ).padStart(8)} | ` +
      ...
      const domItems =
        a['dom-size-insight']?.details?.items ??
        a['dom-size']?.details?.items ??
        []
      if (domItems.length) {
        console.log('\n── dom-size ──')
        for (const it of domItems) {
          console.log(
            `  ${String(it.value?.value ?? it.value).padStart(8)}  ${
              it.statistic
            }`
          )
        }
      ...
      const flat = treemap
        .map((n) => ({
          name: String(n.name).replace(`http://localhost:${PORT}`, ''),
          bytes: n.resourceBytes ?? 0,
        }))
        .sort((x, y) => y.bytes - x.bytes)
      let totalJs = 0
      for (const n of flat) totalJs += n.bytes
      console.log(`  합계 ${kb(totalJs)}`)
      for (const n of flat.slice(0, 10))
        console.log(`  ${kb(n.bytes).padStart(9)}  ${n.name}`)
      ...
      if (third.length) {
        ...
        for (const it of third.slice(0, 5)) {
          console.log(
            `  ${ms(it.blockingTime).padStart(8)} blocking  ${
              it.entity?.text ?? it.entity
            }`
          )
        }
      }
